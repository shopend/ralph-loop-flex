...
export const BASE = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/ralph-api`;
...
export async function createRun(
  ...
  apiKey?: string
): Promise<{ run_id: string; api_key: string; status: string; config: RunConfig }> {
  const res = await fetch(`${BASE}/runs`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${import.meta.env.VITE_SUPABASE_ANON_KEY}` },
    body: JSON.stringify({ prd, config, api_key: apiKey }),
  });
